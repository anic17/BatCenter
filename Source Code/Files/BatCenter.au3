...
; OnAutoItExitRegister(_updateDB)
; #include <Array.au3>
;  _____ _          _           _
; |_   _| |        | |         | |
...
; =======================================================================
Global Const $sAllReposTargetUrl = "https://api.github.com/users/TheBATeam/repos"
...
ElseIf _CmdLine_KeyExists('list') Then
	If IsConnected() Then
		; Updates Local file.
		_dl($sStaticURLtoJSONfile, " > Connecting to GitHub API...           ", $sStaticJSONfilePath_)
		If $hChk($sStaticJSONfilePath_) Then
			List()
		Else
...
Func List()
	...
	Local $i = 0
	Local $sSearchCount = 0 ; Results for unspecified Searchname @ Param -list "Search String."
	While 1
		; ==========================================================================
		...
		Switch $sUsrRequest
			...
			Case Else
				If $hSrchStr($sUsrRequest, $asset_val, 2) Then
					$hCout( _
							" > Found " & $sSearchCount & " match." & @CRLF & _
							"   >> " & $asset_val & @CRLF & _
							"   >> " & $PluginDescription & @CRLF)
					$sSearchCount += 1
				EndIf
...
Func Get()
	...
	While 1
		...
		$i += 1
	WEnd
	If _IsAvalidURL($DirectDownloadLink) And $bCanDownlod Then
		$hCout(" > " & $asset_val & @CRLF & _
		...
		If $hChk($sFileOutPut) And _CmdLine_KeyExists("unzip") Then
			...
			If Not @error Then
				$hCout(" > Extracted " & $nTotalContents & " root item(s) from " & $asset_val & ".zip" & @CRLF & _
